...
public class AllSpeciesHolder : MonoBehaviour {
    ...
    [SerializeField] private SerializableDictionary<Species, Texture> speciesIconDesert = new SerializableDictionary<Species, Texture>();
    ...
    void Start()
    {
        ...
        foreach (var x in speciesIcon) {
            var go = Instantiate(speciesHolderPrefab, transform.position + new Vector3(0, Mathf.Pow(-1f, i%2) * ((i+1)/2) * 120 * canvas.scaleFactor),
        ...
        // Water logo ! 
        var gow = Instantiate(speciesHolderPrefab, transform.position + new Vector3(0, Mathf.Pow(-1f, i%2)*((i+1)/2) * 120 * canvas.scaleFactor),
            speciesHolderPrefab.transform.rotation, transform).transform.GetChild(0);
        ...
        // Tree logo ! 
        gow = Instantiate(speciesHolderPrefab, transform.position + new Vector3(0, Mathf.Pow(-1f, i%2)* ((i+1) / 2) * 120 * canvas.scaleFactor),
            speciesHolderPrefab.transform.rotation, transform).transform.GetChild(0);
